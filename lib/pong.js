    ...
    return pong.findPlayers(player_names).then(function(players) {
      return Q.all(players.map(function (player) {
        ...
        Challenge.findOne({ _id: player.currentChallenge }).then(function (challenge) {
          if (challenge) {
            deferred.reject(new Error("There's already an active challenge between " + challenge.challenger.join(' and ') + ' and ' + challenge.challenged.join(' and ') + '. One of these players can `pongbot chicken` to cancel the challenge.'));
          } else {
            deferred.resolve(player);
          }
        }, function(err) {
