...
module.exports.index = function (req, res) {
  ...
  if (hook) {
    ...
    case 'help':
      res.json({ text: 'https://github.com/Aaron7/slack-pongbot (forked)\n' +
        'Everyone must first `pongbot register`\n' +
        '*To challenge a player:*\n' +
        '`pongbot challenge singles <name>` or `pongbot challenge doubles <team_name> against <opponent_1> <opponent_2>`\n' +
        '*The other player must then either:*\n' +
        '`pongbot accept` or `pongbot decline` or anyone can cancel the challenge by `pongbot chicken`\n' +
        '*The defeated must then declare the result:*\n' +
        '`pongbot lost`\n' +
        '*Other commands:*\n' +
        '`pongbot leaderboard` and `pongbot rank <name>` (with no name to see your own rank)'
      });
      break;
